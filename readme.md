...
Vite 默认监听 `http://localhost:5173`。主站首页为 `http://localhost:5173/workspace/artist/library`。家长展示页使用发布时后端签发的真实 token，形式为 `/#/share/student/{lessonId}/{studentId}?token={token}`；旧文档中的演示 token 不是可用的公开凭证。
...
第五步逐学生微信图文复制与发送确认的使用规则见[家长交付现状与图文复制改造](../docs/家长交付现状与图文复制改造.md)。
